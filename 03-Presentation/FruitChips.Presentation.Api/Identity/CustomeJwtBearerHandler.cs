...
{
    public class CustomeJwtBearerHandler : AuthenticationHandler<JwtBearerOptions>
    {
        private OpenIdConnectConfiguration _configuration;
        private readonly ITokenService _tokenService;
        private readonly SignInManager<Customer> signInManager;

        public CustomeJwtBearerHandler(
        ...
        protected override async Task<AuthenticateResult> HandleAuthenticateAsync()
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(token))
                {
                    ...
                    if (!_tokenService.ExistToken(token))
                        return AuthenticateResult.NoResult();
                }
                ...
                foreach (var validator in Options.SecurityTokenValidators)
                {
                    if (validator.CanReadToken(token))
                    {
                        ...
                        try
                        {
                            ...
                            var userId = Guid.Parse(principal.Claims.FirstOrDefault(x => x.Type == ClaimTypes.NameIdentifier).Value);
                            //var securityStamp = principal.Identity.FindFirstValue(new ClaimsIdentityOptions().SecurityStampClaimType);
                            var validSecurityStamp = await signInManager.ValidateSecurityStampAsync(principal);
                            if (validSecurityStamp is null)
                                throw new Exception("Token Invlaide!!");
                        }
                        catch (Exception ex)
                        {
